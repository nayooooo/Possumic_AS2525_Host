/**
 **************************************************************************************************
 * @file    port_uart_com_isr.c
 * @brief   port communication adaptation for uart, notify type is COM_ISR,
...
static int porting_port_uart2_interrupt_ctrl(void *bus, bool en, uint32_t size);
...
static int porting_port_uart2_interrupt_handle_regist(void *bus, host_com_interrupt_handler_t handler, void *arg);
#endif
...
static const host_com_ops_t uart2_ops = {
    .init                    = porting_port_uart_init,
    .deinit                  = porting_port_uart_deinit,
    .open                    = porting_port_uart_open,
    .close                   = porting_port_uart_close,
    .write                   = porting_port_uart_write,
    .read                    = porting_port_uart_read,
    .interrupt_ctrl          = porting_port_uart2_interrupt_ctrl,
    .interrupt_handle_regist = porting_port_uart2_interrupt_handle_regist,
};
#endif
 ...
 * ------------------------------------------------------------------------------------------------
 */
static HAL_Dev_t *uartDev[PORT_UART_NUM] = {
    [0 ... PORT_UART_NUM-1 ] = NULL
};
static void * const uartHdl[PORT_UART_NUM] = { PORT_UART0_DEV_HANDLE, PORT_UART1_DEV_HANDLE, PORT_UART2_DEV_HANDLE };

static host_com_interrupt_handler_t port_uart_interrupt_handle[PORT_UART_NUM] = {
    [0 ... PORT_UART_NUM-1 ] = NULL
};
static void *port_uart_interrupt_handle_arg[PORT_UART_NUM] = {
    [0 ... PORT_UART_NUM-1 ] = NULL
};
...
static int porting_port_uart2_interrupt_ctrl(void *bus, bool en, uint32_t size)
{
    ...
    if (en) {
        porting_port_uart_set_rx_fifo_threshold(2, size);
        HAL_UART_EnableRxReadyIRQ(uartDev[2]);
    } else {
        HAL_UART_DisableRxReadyIRQ(uartDev[2]);
    }
...
static int porting_port_uart2_interrupt_handle_regist(void *bus, host_com_interrupt_handler_t handler, void *arg)
{
    port_uart_interrupt_handle[2] = handler;
    port_uart_interrupt_handle_arg[2] = arg;
...
host_com_ops_t *porting_port_uart_get_com_ops(uint8_t id)
{
    ...
    if (id == 0) {
        ...
        return (host_com_ops_t *)&uart2_ops;
    } else {
        return NULL;
    }
#else
